...
routes.get("/allpost", requireLogin, (req, res) => {
  Post.find()
...
routes.put("/like", requireLogin, (req, res) => {
...
routes.put("/unlike", requireLogin, (req, res) => {
  Post.findByIdAndUpdate(
    req.body.postId,
    {
...
routes.put("/comment", requireLogin, (req, res) => {
  const comment = {
    text: req.body.text,
    ...
    .exec((err, post) => {
      ...
      if (post.postedBy._id.toString() === req.user._id.toString()) {
        post.remove().then((result) => {
          res.json({ message: "Successfully Deleted" }).catch((error) => {
            res.json({ error });
          });
        });
      }
