...
const Home = () => {
    ...
    useEffect(() => {
        fetch('/getsubpost', {
            ...
            .then(res => res.json())
            .then(res => {
                // console.log("all my follower post data",res);
                setData(res.post)
            })
    }, [data])
    ...
    return (
        <>
            <div className="home">
                <p style={{padding:"10px",fontWeight:"500"}}>People You Follow...</p>
                {
                    data.map(item => {
                        return (<div className="card home-card" key={item._id}>
                            <div style={{display:'flex',justifyContent:"space-between"}}>
                              <NavLink exact to={`/profile/${item.postedBy._id==state._id?"":item.postedBy._id}`}><h5 style={{ padding: "10px", fontWeight: "bold" }} 
                              >{item.postedBy.name}</h5> </NavLink>
                            {item.postedBy._id == state._id && <i className="material-icons favroite" style={{float:"right"}} onClick={() => deletePost(item._id)} >delete</i>}
                            </div>
